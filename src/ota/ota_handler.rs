...
impl OtaHandler {
        ...
        P: Publisher + Send + Sync + 'static,
    {
        ...
        let publisher = OtaPublisher { client };
    ...
    pub async fn handle_event(&mut self, req: OtaRequest) -> Result<(), DeviceManagerError> {
        let id = match OtaId::try_from(&req) {
            Ok(id) => id,
            Err(err) => {
                let _ = self
                    .publisher_tx
                    .send(OtaStatus::Failure(err.clone(), None))
                    .await;

                return Err(DeviceManagerError::Ota(err));
            }
        };

        match req.operation {
            OtaOperation::Update => self.handle_update(id).await,
            OtaOperation::Cancel => self.handle_cancel(id).await,
        }
    }

    async fn handle_update(&mut self, id: OtaId) -> Result<(), DeviceManagerError> {
        self.check_update_already_in_progress(&id).await?;
    ...
    async fn check_update_already_in_progress(
        ...
        // OTA no longer in progress
        if !self.flag.in_progress() {
            self.current = None;
        ...
        match &self.current {
            Some(current) if current.ota_id == *id => Ok(()),
            Some(current) => {
                debug!(
                ...
                return Err(DeviceManagerError::Ota(OtaError::UpdateAlreadyInProgress));
            }
    ...
    async fn handle_cancel(&mut self, id: OtaId) -> Result<(), DeviceManagerError> {
        match &self.current {
