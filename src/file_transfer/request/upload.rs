...
impl<'a> TryFrom<&'a DeviceToServer> for Upload<'a> {
    ...
    fn try_from(value: &'a DeviceToServer) -> Result<Self, Self::Error> {
        ...
        let source = Target::try_from(value)?;
...
impl<'a> TryFrom<&'a DeviceToServer> for Source<'a> {
    type Error = eyre::Report;

    fn try_from(value: &'a DeviceToServer) -> Result<Self, Self::Error> {
        let source_type = value.source_type.as_str();
        let source = value.source.as_str();

        match source_type {
            STORAGE_TARGET => Ok(Self::Storage {
                id: value.source.parse()?,
            }),
            STREAMING_TARGET => {
                if !source.is_empty() {
...
pub(crate) mod tests {
    ...
    #[case(Target::Storage)]
    #[case(Target::Stream)]
    #[case(Target::FileSystem{ path: Cow::Borrowed(Path::new("/etc/config")) })]
    fn targets_roundtrip(#[context] ctx: Context, #[case] target: Target) {
