...
impl<'a> TryFrom<&'a ServerToDevice> for Download<'a> {
    ...
    fn try_from(value: &'a ServerToDevice) -> Result<Self, Self::Error> {
        ...
        let destination = Target::try_from(value)?;
...
impl<'a> Destination<'a> {
    fn from_str(destination_type: &str, destination: &'a str) -> eyre::Result<Self> {
        match destination_type {
            ...
            FILESYSTEM_TARGET => Ok(Target::FileSystem {
...
impl<'a> TryFrom<&'a ServerToDevice> for Destination<'a> {
    type Error = eyre::Report;

    fn try_from(value: &'a ServerToDevice) -> Result<Self, Self::Error> {
        let destination = &value.destination;
        let destination_type = value.destination_type.as_str();

        Self::from_str(destination, destination_type)
    }
}
