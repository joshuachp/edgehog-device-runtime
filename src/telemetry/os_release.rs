...
impl OsInfo {
    const INTERFACE: &'static str = "io.edgehog.devicemanager.OSInfo";
...
impl BaseImage {
        ...
        T: Publisher,
    {
        match self.name {
            Some(name) => {
                publish_prop(client, Self::INTERFACE, "/name", name).await;
            }
            None => {
        ...
        match self.version {
            Some(version) => {
                publish_prop(client, Self::INTERFACE, "/version", version).await;
            }
            None => {
        ...
        match self.build_id {
            Some(build_id) => {
                publish_prop(client, Self::INTERFACE, "/buildId", build_id).await;
            }
            None => {
...
"#;
        ...
        assert_eq!(data.name.unwrap(), "Arch Linux");
        assert_eq!(data.version.unwrap(), "rolling");
...
BUG_REPORT_URL="https://bugs.debian.org/""#;
        ...
        let data = OsRelease::from(file).os_info;
        assert_eq!(data.name.unwrap(), "Debian GNU/Linux");
        assert_eq!(data.version.unwrap(), "11");
    }
