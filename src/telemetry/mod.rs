...
use crate::controller::message::{TelemetryConfig, TelemetryEvent};
...
use sender::{Task, TelemetryInterface};
...
impl TaskConfig {
    ...
    fn from_config<'a>(config: &TelemetryInterfaceConfig<'a>) -> Self {
        Self {
    ...
    fn from_override<'a>(over: &TelemetryInterfaceConfig<'a>) -> Option<Self> {
...
impl<T> Telemetry<T> {
        ...
        T: Publisher,
    {
        ...
        HardwareInfo::new(&self.client).send().await;

        runtime_info::send_runtime_info(&self.client).await;
...
mod tests {
    ...
    use crate::repository::file_state_repository::FileStateRepository;
    use crate::repository::StateRepository;
    use crate::telemetry::sender::TelemetryInterface;
