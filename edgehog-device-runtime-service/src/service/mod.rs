...
async fn serve<S, IO>(this: Arc<EdgehogService>, incoming: S) -> eyre::Result<()>
where
    ...
    IO: AsyncRead + AsyncWrite + Connected + Unpin + Send + 'static,
{
    Server::builder()
        .add_service(
            edgehog_proto::containers::latest::containers_service_server::ContainersServiceServer::from_arc(Arc::clone(&this)),
        )
        .serve_with_incoming_shutdown(incoming, async { todo!() }).await?;
