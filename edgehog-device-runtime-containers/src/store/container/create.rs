...
impl StateStore {
    ...
    // XXX: separate in subfunction, this is too long
    ...
    pub(crate) async fn create_container(
        ...
        self.handle
            .for_write(move |writer| {
                ...
                let storage_opt =
                    zip_opt_vecs(storage_opt_keys, storage_opt_values, "storage options")?;
                ...
                let tmpfs = zip_opt_vecs(tmpfs_paths, tmpfs_options, "tmpfs options")?;
                ...
                let labels = zip_opt_vecs(labels_keys, labels_values, "labels")?;
                ...
                let ulimits = zip3_opt_vecs(ulimits_name, ulimits_soft, ulimits_hard, "ulimits")?;
                ...
                let sysctls = zip_opt_vecs(sysctls_keys, sysctls_values, "sysctl")?;
                ...
                let log_config = zip_opt_vecs(log_config_keys, log_config_values, "log config")?;
                ...
                let blkio_device_weight = zip_opt_vecs(
                    blkio_weight_device_path,
                    blkio_weight_device_weight,
                    "blkio weight device",
                )?;
                ...
                let blkio_device_read_bps = zip_opt_vecs(
                    blkio_device_read_bps_path,
                    blkio_device_read_bps_rate,
                    "blkio device read",
                )?;
                ...
                let blkio_device_write_bps = zip_opt_vecs(
                    blkio_device_write_bps_path,
                    blkio_device_write_bps_rate,
                    "blkio device write",
                )?;
                ...
                let blkio_device_read_iops = zip_opt_vecs(
                    blkio_device_read_iops_path,
                    blkio_device_read_iops_rate,
                    "blkio device read iops",
                )?;
                ...
                let blkio_device_write_iops = zip_opt_vecs(
                    blkio_device_write_iops_path,
                    blkio_device_write_iops_rate,
                    "blkio device write iops",
                )?;
...
impl<'a> TryFrom<&'a CreateContainer> for container::Container<'a> {
    ...
    fn try_from(value: &'a CreateContainer) -> Result<Self, Self::Error> {
        ...
        Ok(container::Container {
            ...
            cpuset_cpus: cpuset_cpus.as_ref().map(Cow::from),
...
type ZipIter<'a, T, U> = std::iter::Zip<slice::Iter<'a, T>, slice::Iter<'a, U>>;

fn zip_opt_vecs<'a, T, U>(
    keys: &'a Option<Vec<T>>,
    values: &'a Option<Vec<U>>,
    ctx: &'static str,
) -> Result<ZipIter<'a, T, U>, HandleError> {
    let keys = map_opt_vec(keys);
    let values = map_opt_vec(values);

    if keys.len() == values.len() {
        Ok(keys.iter().zip(values))
    } else {
        Err(HandleError::Application(
            StoreError::Conversion {
                ctx: format!(
                    "couldn't zip {ctx} keys({}) and values({})",
                    keys.len(),
                    values.len()
                ),
            }
...
type Zip3Iter<'a, T, U, Z> =
    std::iter::Zip<std::iter::Zip<slice::Iter<'a, T>, slice::Iter<'a, U>>, slice::Iter<'a, Z>>;

fn zip3_opt_vecs<'a, T, U, Z>(
    a: &'a Option<Vec<T>>,
    b: &'a Option<Vec<U>>,
    c: &'a Option<Vec<Z>>,
    ctx: &'static str,
) -> Result<Zip3Iter<'a, T, U, Z>, HandleError> {
    let a = map_opt_vec(a);
    let b = map_opt_vec(b);
    let c = map_opt_vec(c);

    if a.len() == b.len() && b.len() == c.len() {
        Ok(a.iter().zip(b).zip(c))
    } else {
        Err(HandleError::Application(
            StoreError::Conversion {
                ctx: format!(
                    "couldn't zip {ctx} the three have different lengths ({}, {}, {})",
                    a.len(),
                    b.len(),
                    c.len()
                ),
...
pub(crate) mod tests {
    ...
    pub(crate) fn stored_container_full(id: Uuid, image: &CreateImage) -> Container<'static> {
        Container {
            ...
            cpu_shares: Some(100),
            cpuset_cpus: Some(Cow::Borrowed("0-3")),
            memory: Some(4096),
