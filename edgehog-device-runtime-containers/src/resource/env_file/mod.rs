...
use self::parser::{EnvError, EnvReader};
...
impl<'a> EnvFileResource<'a> {
        ...
        D: PropAccess + Send + Sync + 'static,
    {
        ...
        let mut file = match tokio::fs::File::open(path_on_device).await {
            Ok(file) => EnvReader::new(file),
        ...
        file.validate().await.map_err(|err| match err {
            EnvError::Io(error) => {
    ...
    pub(crate) async fn path_on_device<D>(&self, ctx: &Context<'_, D>) -> Result<PathBuf>
    where
        D: PropAccess + Send + Sync + 'static,
    {
        match self.inner.target_type {
            TargetType::Storage => {
                file_store_device_path(ctx, "env file", ctx.id, &self.inner.target_id).await
            }
        }
    }
}
    ...
    async fn should_get_path_on_device() {
        ...
        device
            ...
            .with(
                ...
                predicate::eq(format!("/{}/pathOnDevice", env_file.id)),
            )
            .returning({
